...
private:
    ...
    std::vector<pcl::PointXYZ, Eigen::aligned_allocator<pcl::PointXYZ>> pointsMade;
...
public:
   ...
   */
    Cloud2Depth() : it_(nh_)
    {
        ...
        sub_ = nh_.subscribe("/aditof_roscpp/aditof_pcloud", 5, &Cloud2Depth::cloudCallback, this);
        // sub2_ = it_.subscribe("/aditof_depth_32FC1_format", 5, &Cloud2Depth::depthCallback, this);
        sub2_ = it_.subscribe("/aditof_roscpp/aditof_depth", 5, &Cloud2Depth::depthCallback, this);
    ...
    void cloudCompare() //const sensor_msgs::ImageConstPtr *img_msg)
    {
        cloudReceived = false;
        cloudMadeReceived = false;
    ...
    void cloudCallbackMade(const PointCloud::ConstPtr &cloud_in_made_msg)
    {
        if (!cloud_in_msg || cloud_in_made_msg->size() <= 0)
        {
            ROS_WARN("got empty or invalid pointcloud --> ignoring");
            return;
        }
        pointsMade = cloud_in_made_msg->points;
        // &global_cloud_in = &cloud_in_msg;
        cloudMadeReceived = true;
        if (cloudMadeReceived == true && cloudReceived == true)
        {
            cloudCompare();
        }
        // waitKey(3);
