...
class I2D(nn.Module):
    ...
    def _upsample_add(self, x, y):
        ...
        '''
        _,_,H,W = y.size()
        return F.upsample(x, size=(H,W), mode='bicubic') + y
    ...
    def forward(self, x):
        ...
        _,_,H,W = d2.size()
        vol = torch.cat( [ F.upsample(d, size=(H,W), mode='bicubic') for d in [d5,d4,d3,d2] ], dim=1 )
        
        # return self.predict2( self.up4(self.predict1(vol)) )
        pred1 = self.predict1(vol)
        pred2 = F.interpolate(self.predict2(pred1), size=(H*4,W*4), mode='bicubic')
        valid = x[0][1]!=0.0
        pred3 = pred2.clone()
