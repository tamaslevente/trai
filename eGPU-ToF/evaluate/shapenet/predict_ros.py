...
# sys.path.append('/home/rambo/pvcnn_clean_test')
...
def predict(image_msg_depth, image_msg_ir):
    shape_index = 0
    ##############
    # prediction #
    ##############
    ...
    # gen = point_cloud2.read_points(image_msg, field_names=("x", "y", "z"), skip_nans=True)

    # points_list = []

    # for data in pc2.read_points(image_msg, skip_nans=True):
    #     points_list.append([data[0], data[1], data[2], data[3]])

    # pcl_data = pcl.PointCloud_PointXYZRGB()
    # pcl_data.from_list(points_list)

    pc = ros_numpy.numpify(image_msg_depth)
    height = pc.shape[0]
    width = pc.shape[1]
    shape_index = shape_index + 1
    
    temp_x = np.resize(pc['x'], height * width)
    temp_x = temp_x[~np.isnan(temp_x)]
    np_points = np.zeros((len(temp_x), 3), dtype=np.float32)
    np_points[:, 0] = temp_x

    temp_y = np.resize(pc['y'], height * width)
    temp_y = temp_y[~np.isnan(temp_y)]
    np_points[:, 1] = temp_y 

    temp_z = np.resize(pc['z'], height * width)
    temp_z = temp_z[~np.isnan(temp_z)]
    np_points[:, 2] = temp_z 
    ...
    with torch.no_grad():
        ...
        if True:
            ...
            vote_confidences_x_np = np.mean(vote_confidences_x_np)

            # print('vote_confidences_x_np.shape=',vote_confidences_x_np.shape)
            print('Confidence=', vote_confidences_x_np)
    ...
    if PREDICT_ONE:
        ...
        update_stats(stats, ground_truth, predictions, 0, start_class, end_class)
...
def proc_predict(shape_index,vote_predictions_x_np,vote_confidences_x_np,inputs_np,class2shape_tbl,predict_shape=None):
  ...
  unique, counts = np.unique(vote_predictions_x_np, return_counts=True)
  print(dict(zip(unique, counts)))
  
  # print('vote_confidences_x_np.shape=',vote_predictions_x_np.shape)
...
if __name__ == '__main__':
    ...
    # # subscribe depth img
    # depth = message_filters.Subscriber("/pico_zense/depth/image_raw", Image)
    # # subscribe IR img
    # ir = message_filters.Subscriber("/pico_zense/ir/image_raw", Image)

    # ts = message_filters.ApproximateTimeSynchronizer([depth, ir], 10, 60, allow_headerless=False)
    # ts.registerCallback(predict)

    _ = rospy.Subscriber("/tof/depth/points", PointCloud2, predict)
