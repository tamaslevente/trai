...
def test(model, loader, opt):
    ...
    for child in model_children:
        ...
        if type(child) == gcn_lib.dense.DenseDilatedKnnGraph:
            layers.append(child)
            #print("DilatedKNN: " + str(type(child)))
        if type(child) == gcn_lib.dense.GraphConv2d:
            layers.append(child)
            #print("GraphConv2D: " + str(type(child)))
        if type(child) == gcn_lib.dense.BasicConv:
            layers.append(child)
            #print("BasicConv: " + str(type(child)))      
        if type(child) == torch.nn.Sequential:
            #print("Sequential: " + str(type(child)))
            for idx, nephew in enumerate(child.children()):
                if type(nephew) == gcn_lib.dense.ResDynBlock2d:
                    layers.append(nephew)
                if type(nephew) == gcn_lib.dense.BasicConv:
                    layers.append(nephew)
                    #print("Basic conv: " + str(nephew))
                if type(nephew) == torch.nn.Dropout:
                    layers.append(nephew)
    ...
    with torch.no_grad():
        ...
        # Recreating forward from DeepGCN
        outputs = []
        # Effectively creating the layers in a similar format to DeepGCN
        knn = layers.pop(0)
        head = layers.pop(0)
        backbone = layers[0:27]
        fusion_block = layers[27]
        final_layers = layers[28:]
        # Utilizing the layers
        dilated = knn(inputs[:, 0:3])
        print("Dilated: ")
        print(dilated.shape)
        feats = [head(inputs, dilated)]
        print("Head: ")
        print(feats[0].shape)
        print("Backbone: ")
        for idx, layer in enumerate(backbone):
            feats.append(layer(feats[-1]))   
        print(str(feats[-1].shape) + ", but 27 times")

        print("Concatenated post-backbone: ")
        feats = torch.cat(feats, dim=1)
        print(feats.shape)

        
        fusion = torch.max_pool2d(fusion_block(feats), kernel_size=[feats.shape[2], feats.shape[3]])
        print("Max_pool2d with fusion_block: ")
        print(fusion.shape)
        fusion = torch.repeat_interleave(fusion, repeats=feats.shape[2], dim=2)
        print("Repeat_interleave fusion: ")
        print(fusion.shape)
        result = final_layers[0](torch.cat((fusion, feats), dim=1))
        print("BasicConv 1: ")
        print(result.shape)
        result = final_layers[1](result)
        print("BasicConv 2: ")
        print(result.shape)
        result = final_layers[2](result)
        print("Dropout: ")
        print(result.shape)
        result = final_layers[3](result)
        print("Result, after a basic conv: ")
