...
int main(int argc, char **argv)
{
    ...
    char file_ir[200];
    sprintf(file_ir, "%s%s", irdir.c_str(), nfilename.c_str());
    char file_xyz[200];
    dfilename = dfilename.substr(0, dfilename.size() - 10);
    sprintf(file_xyz, "%s%s.xyz", pcddir.c_str(), dfilename.c_str());
    ...
    for (int i = 0; i < depth.rows; i++)
    {
        for (int j = 0; j < depth.cols; j++)
        {
            ...
            float ir_val = ir.at<uint16_t>(i, j);
            
            myfile << p.x << " " << p.y << " " << p.z << " " << 3 <<"\n";
            // myfile << p.x << " " << p.y << " " << p.z << " " << ir_val << " " << 3 <<"\n";
        }
    }
