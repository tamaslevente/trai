...
public:
      ...
      : private_nh("~")
  {
    ...
    pub_segment= nh_.advertise<pcl_msgs::PolygonMesh>("pcd_segment_mesh",10);
    ...
    label_pub = nh_.advertise<std_msgs::String>("GT", 1);
  ...
  ret_sphere(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud)
  {
    ...
    model->setRadiusLimits(0.0, 0.20);
  ...
  // pcl::PointCloud<pcl::PointXYZ>::Ptr
  // ret_parallel_line(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud)
  // {
  //   cout<<"------------------------"<<endl;
  ...
  //   pcl::ModelCoefficients::Ptr coefficients_plane(new pcl::ModelCoefficients), coefficients_parallel_line(new pcl::ModelCoefficients);
  //   pcl::PointIndices::Ptr inliers_plane(new pcl::PointIndices), inliers_parallel_line(new pcl::PointIndices);
  ...
  //   // Create the segmentation object for parallel line and set all the parameters
  //   seg.setOptimizeCoefficients(true);
  //   seg.setMethodType(pcl::SAC_RANSAC);
  //   seg.setModelType(pcl::SACMODEL_PARALLEL_LINE);
  //   seg.setDistanceThreshold(0.2);
  //   seg.setAxis(Eigen::Vector3f(0, 0, 1)); // Vertical axis
  //   seg.setEpsAngle(pcl::deg2rad(5.0));
  //   seg.setInputCloud(cloud_filtered2);
  //   seg.setInputNormals(cloud_normals2);
  //   seg.segment(*inliers_parallel_line, *coefficients_parallel_line);
  //   extract.setInputCloud(cloud);
  //   extract.setIndices(inliers_parallel_line);
  //   extract.setNegative(false);

  //   cout<<"----------cloud->size()--------------"<<cloud->size()<<endl;
    
  //   pcl::PointCloud<PointT>::Ptr cloud_parallel_line(new pcl::PointCloud<PointT>());
  //   extract.filter(*cloud_parallel_line);

  //   cout<<*cloud_parallel_line<<"-----------cloud_parallel_line-------------";
  //   cout<<"------------------------";
  //   return cloud_parallel_line;
  // }
  ...
  // pcl::PointCloud<pcl::PointXYZ>::Ptr
  // ret_line(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud)
  // {
  ...
  //   pcl::PointIndices::Ptr inliers(new pcl::PointIndices);
  //   pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients);
  //   pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_parallel_line(new pcl::PointCloud<pcl::PointXYZ>());
  //   seg.setOptimizeCoefficients(true);
  //   seg.setMethodType(pcl::SAC_RANSAC);
  //   seg.setModelType(pcl::SACMODEL_PARALLEL_LINE);
  //   seg.setDistanceThreshold(0.2);
  //   seg.setAxis(Eigen::Vector3f(0, 1, 0));  /* axis y */
  //   seg.setEpsAngle(pcl::deg2rad(5.0));
  //   seg.setInputCloud(cloud);
  //   seg.segment(*inliers, *coefficients);
  //   pcl::ExtractIndices<pcl::PointXYZ> extract;
  ...
  shapefind(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cluster, int j)
  {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_pl(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    // cloud_pl = ret_parallel_line(cloud_cluster);
    // cloud_l = ret_line(cloud_pl);

    // cout<<cloud_pl->size()<<"cloud_pl->size()---------->";

    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_pl_cluster(new pcl::PointCloud<pcl::PointXYZRGB>);
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_all(new pcl::PointCloud<pcl::PointXYZRGB>); 

    // pcl::copyPointCloud(*cloud_l, *cloud_pl_cluster);
    // pcl::copyPointCloud(cloud_cluster, *cloud_all);

    // std::stringstream name;
    // std::stringstream name1;
    // name << "/media/cuda/ssd/datasets/ModelNet_own/parallel_line/pcd/parallel_line_" << std::to_string(cloud_l->size()) << ".pcd";
    // name1 << "/media/cuda/ssd/datasets/ModelNet_own/parallel_line/pcd/parallel_" << std::to_string(cloud_l->size()) << ".pcd";
    // nr_of_parallel_line++;
    // cout << name << endl;
    // if (cloud_pl->size() != 0)
    // {
    //   writer.write<pcl::PointXYZRGB>(name.str(), *cloud_pl_cluster, false);
    //   writer.write<pcl::PointXYZRGB>(name1.str(), *cloud_all, false);
    // }
    ...
    if ((overlap_p > overlap_s) && (overlap_p > overlap_c) && (overlap_p > th) && (size_cloud_p > min_cloud_size))
    {
      ...
      // float angle = gravity.dot(normal);
    ...
    case 1:
    {
      ...
      if (big_plane)
      {
        ...
        // std::stringstream name;
        // name << "/media/cuda/ssd/datasets/ModelNet_own/big_plane/pcd/big_plane_" << std::to_string(nr_of_spheres) << ".pcd";
        // nr_of_spheres++;
        ...
        if (vertical)
        {
          ...
          // std::stringstream name;
          // name << "/media/cuda/ssd/datasets/ModelNet_own/vertical_plane/pcd/vertical_plane_" << std::to_string(nr_of_vertical_plane) << ".pcd";
          // nr_of_vertical_plane++;
          // //std::cout << name << std::endl;
          // writer.write<pcl::PointXYZRGB>(name.str(), *cloud_colored_cluster, false);
        ...
        if (horizontal_floor)
        {
          ...
          // std::stringstream name;
          // name << "/media/cuda/ssd/datasets/ModelNet_own/horizontal_plane/pcd/horizontal_plane_" << std::to_string(nr_of_horizontal_plane) << ".pcd";
          // nr_of_horizontal_plane++;
          // //std::cout << name << std::endl;
          // writer.write<pcl::PointXYZRGB>(name.str(), *cloud_colored_cluster, false);
      ...
      else
      {
        ...
        // std::stringstream name;
        // name << "/media/cuda/ssd/datasets/ModelNet_own/small_plane/pcd/small_plane_" << std::to_string(nr_of_small_plane) << ".pcd";
        // nr_of_small_plane++;
        // //std::cout << name << std::endl;
        // writer.write<pcl::PointXYZRGB>(name.str(), *cloud_colored_cluster, false);
    ...
    case 2:
    {
      ...
      std::stringstream name;
      name << "/media/cuda/ssd/datasets/ModelNet_own/sphere/pcd/sphere_" << std::to_string(nr_of_spheres) << ".pcd";
      nr_of_spheres++;
      //std::cout << name << std::endl;
      writer.write<pcl::PointXYZRGB>(name.str(), *cloud_colored_cluster, false);
    ...
    case 3:
    {
      ...
      std::stringstream name;
      name << "/media/cuda/ssd/datasets/ModelNet_own/cylinder/pcd/cylinder_" << std::to_string(nr_of_cylinder) << ".pcd";
      nr_of_cylinder++;
      //std::cout << name << std::endl;
      writer.write<pcl::PointXYZRGB>(name.str(), *cloud_colored_cluster, false);
  ...
  planefind(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cluster, int j, pcl::ModelCoefficients::Ptr coefficients)
  {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_pl(new pcl::PointCloud<pcl::PointXYZ>);
    // cloud_pl = ret_parallel_line(cloud_cluster);
    // cout<<cloud_pl->size()<<"cloud_pl->size()-----planefind----->"<<endl;
    ...
    if (big_plane)
    {
      ...
      if (horizontal_floor)
      {
        ...
        // std::stringstream name;
        // name << "/media/cuda/ssdls/datasets/ModelNet_own/horizontal_plane/pcd/horizontal_plane_" << std::to_string(nr_of_horizontal_plane) << ".pcd";
        // nr_of_horizontal_plane++;
        // //std::cout << name << std::endl;
        // writer.write<pcl::PointXYZRGB>(name.str(), *cloud_colored_cluster, false);
  ...
  void
  createMesh(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cluster, std_msgs::String msg){
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_mesh (new pcl::PointCloud<pcl::PointXYZ>);
      //pcl::fromPCLPointCloud2 (cloud_cluster, *cloud_mesh);
      cloud_mesh=cloud_cluster;
      pcl::NormalEstimation<pcl::PointXYZ, pcl::Normal> nm;
      pcl::PointCloud<pcl::Normal>::Ptr normalsm (new pcl::PointCloud<pcl::Normal>);
      pcl::search::KdTree<pcl::PointXYZ>::Ptr treem (new pcl::search::KdTree<pcl::PointXYZ>);
      treem->setInputCloud (cloud_mesh);
      nm.setInputCloud (cloud_mesh);
      nm.setSearchMethod (treem);
      nm.setKSearch (20);
      nm.compute (*normalsm);
      pcl::PointCloud<pcl::PointNormal>::Ptr cloud_with_normals (new pcl::PointCloud<pcl::PointNormal>);
      pcl::concatenateFields (*cloud_mesh, *normalsm, *cloud_with_normals);
      ...
      //printf("Publishing segmentations");
      pub_segment.publish(pcl_msg_mesh);
  ...
  clusterfind(const PointCloud::ConstPtr &cloud_in)
  {
    ...
    // uncomment if azure Kinect DK camera is used
...
private:
  ...
  int nr_of_spheres = 0;
  int nr_of_big_plane = 0;
  int nr_of_small_plane = 0;
  int nr_of_vertical_plane = 0;
  int nr_of_horizontal_plane = 0;
  int nr_of_parallel_line = 0;
