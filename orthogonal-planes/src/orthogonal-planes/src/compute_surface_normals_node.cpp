...
#include <ppfplane/compute_surface_normals_nodeConfig.h>
...
public:
  ...
  ComputeSurfaceNormalNode()
  {
    ...
    pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/norm_out",1);
                              ...
                              pcl::PointCloud<pcl::PointNormal> &cloudnormals)
{
  ...
  void
  dynReconfCallback(ppfplane::compute_surface_normals_nodeConfig &config, uint32_t level)
  {
  ...
  cloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloud_msg)
  {
    ...
    *cloudPTR = cloud_Test;
    ...
    pcl::toROSMsg(cloudnormals, tempROSMsg);
...
private:
  ...
  dynamic_reconfigure::Server<ppfplane::compute_surface_normals_nodeConfig> config_server_;
