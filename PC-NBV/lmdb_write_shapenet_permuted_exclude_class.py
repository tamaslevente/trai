                ...
                for model_id in model_list:
                    ...
                    for ex_index in range(self.ex_times):
                        for scan_index in range(self.num_scans):
                            ...
                            view_state = np.load(os.path.join("/home/cuda/Alex/trai/PC-NBV/NBV_data/shapenet_33_views_640x480/test_permuted/", str(model_id), str(ex_index), str(scan_index) + "_viewstate_permuted.npy"))
                            target_value=np.load(os.path.join("/home/cuda/Alex/trai/PC-NBV/NBV_data/shapenet_33_views_640x480/test_permuted/", str(model_id), str(ex_index), str(scan_index) + "_target_value_permuted.npy")) 
                            yield model_id, accumulate_pointcloud, gt_pc, view_state, target_value
                print(class_id+" "+str(i)+" models")
