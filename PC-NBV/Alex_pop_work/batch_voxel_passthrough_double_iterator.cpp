...
int main(int argc, char **argv)
{
  ...
  std::ifstream Output_Folder("../config_files/output_folder.txt");
...
std::getline(Output_Folder, output_folder_path);
  ...
  for (auto& p : fs::directory_iterator(path))
   ...
   fs::path path2(str);
	...
	for (auto& p2 : fs::directory_iterator(path2))
	{
	...
	std::string final_path=str2+"/1.pcd";
	...
	//std::cout<<final_path<<'\n';
		  ...
		  if (pcl::io::loadPCDFile<pcl::PointXYZ>(final_path, *cloud) == -1) //* load the file
		  {
		  ...
		  downsample(cloud_2,leaf_size,downsampled);
			...
			*cloud_in_3 = cloud_out2;
			...
			std::string out_file = final_path.substr (string_pointcloud.length(),final_path.length()); 
			
			std::string out_file_final=output_folder_path+out_file;
		   
			std::cout<<out_file_final<<'\n';
			...
			pcl::io::savePCDFileASCII(out_file_final, cloud_out3);
